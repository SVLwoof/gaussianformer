"""End-to-end tomatoes scene pipeline.

For each N in N_VARIANTS:
  1. Prune the (already-scored) normalized Gaussians to top-N by LightGaussian score.
  2. Write pruned PLY and H5 (14-dim tokens + 14-view orbit cameras).
  3. Render the pruned scene via gsplat (column 2: pruned-N ground truth).
  4. Render the same H5 through GaussianFormer v6 (column 3: model output).
  5. Build a per-N 3-way overview: full-scene gsplat | pruned-N gsplat | GF.

The full-scene gsplat reference (column 1) reuses the renders produced earlier
by render_tomatoes_raw_orbit.py — same normalization, same orbit rig, so the
columns line up pixel-for-pixel by camera index.

Outputs land under data_external/tomatoes/{,h5/,renders/}.
"""
...
SCENE_DIR = Path("data_external/tomatoes")
RAW_PLY = SCENE_DIR / "raw.ply"
H5_DIR = SCENE_DIR / "h5"
RENDERS_DIR = SCENE_DIR / "renders"
FULL_GSPLAT_DIR = RENDERS_DIR / "gsplat_full"  # produced by render_tomatoes_raw_orbit.py
CHECKPOINT = Path("checkpoints_v6/phase2_epoch_100.pt")

NORM_SCALE = 2.0  # raw AABB max ~0.225 -> normalized ~0.45
N_VIEWS = 14
RESOLUTION = 512
ORBIT_RADIUS = 1.7
ORBIT_FOV_DEG = 45.0
SCORE_VIEWS = 64
SCORE_RESOLUTION = 512
GAMMA = 0.1  # LightGaussian volume exponent

N_VARIANTS = [5000, 10000, 20000, 30000]


# ------------------------------ Utilities --------------------------------

def normalize_raw(raw_ply: Path) -> dict[str, np.ndarray]:
    """Load raw.ply, apply: median-center -> 180deg-X-rotation -> x2 scale.

    The 180deg rotation about X maps (x,y,z) -> (x,-y,-z) in positions and
    composes the corresponding quaternion update so Gaussian shapes remain
    correct. This flips the source PLY's gravity-down (+Y in source frame)
    to +Y up (standard convention), so downstream tools using up_axis="y"
    frame the bowl right-side-up.
    """
    print(f"Loading {raw_ply}...")
    plydata = PlyData.read(str(raw_ply))
    v = plydata["vertex"]
    n = v.count
    print(f"  {n:,} Gaussians")
    ...
    means *= NORM_SCALE
    scales = np.exp(scales_log) * NORM_SCALE

    print(f"  median {median.tolist()} -> centered, X-rotated, x{NORM_SCALE} scaled")
    ...
    return {
        ...
        "raw_vertex_dtype": v.data.dtype,
        "raw_vertex_data": v.data,
    }


def lightgaussian_scores(arrays: dict, device: torch.device) -> np.ndarray:
    ...
    viewmats_np, Ks_np = make_orbit_views(
        SCORE_VIEWS, ORBIT_RADIUS, ORBIT_FOV_DEG, SCORE_RESOLUTION, up_axis="y"
    )
    ...
    print(f"\nScoring {SCORE_VIEWS} orbit views @ {SCORE_RESOLUTION}px (LightGaussian)...")
    for v in range(SCORE_VIEWS):
        with torch.no_grad():
            _, _, info = gsplat.rasterization(
                means=means, quats=quats, scales=scales,
                opacities=opacities, colors=colors,
                viewmats=viewmats[v:v+1], Ks=Ks[v:v+1],
                width=SCORE_RESOLUTION, height=SCORE_RESOLUTION,
        ...
        if (v + 1) % 16 == 0 or v == SCORE_VIEWS - 1:
            n_seen = int((score > 0).sum().item())
            print(f"  view {v + 1:3d}/{SCORE_VIEWS}  cumulative seen: {n_seen:,} Gaussians")
...
def render_gaussianformer(h5_path: Path, pipeline, device: torch.device) -> np.ndarray:
...
def make_3way_overview(full_imgs: np.ndarray, pruned_imgs: np.ndarray,
                        gf_imgs: np.ndarray) -> np.ndarray:
    """Per-view triple stacked into 7 rows x 2 columns."""
    n = full_imgs.shape[0]
    H = pruned_imgs.shape[1]
    ...
    for i, img in enumerate(triples):
        r, c = divmod(i, cols)
        y, x = r * (Hh + 8), c * (Ww + 8)
        overview[y:y+Hh, x:x+Ww] = img
    return np.clip(overview, 0, 1)
...
def main():
    ...
    parser.add_argument("--tag", type=str, default=None,
                        help="Suffix appended to render output dirs (e.g. 'v9_ep5'). "
                        "When set, renders go under renders/gaussianformer_n{N}_{tag}/ "
                        "and overviews to overview_3way_n{N}_{tag}.png. Default: no tag, "
                        "overwrites the existing v6 outputs.")
    ...
    # Step 1: normalize.
    arrays = normalize_raw(RAW_PLY)
    n_total = len(arrays["means"])

    normalized_ply = SCENE_DIR / "normalized.ply"
    ...
    # Step 2: score once on the full set.
    scores = lightgaussian_scores(arrays, device)
    rank = np.argsort(-scores)  # descending

    # Step 3: cameras (shared across all N variants).
    viewmats_np, Ks_np = make_orbit_views(N_VIEWS, ORBIT_RADIUS, ORBIT_FOV_DEG, RESOLUTION, up_axis="y")
    viewmats = torch.from_numpy(viewmats_np).to(device)
    Ks = torch.from_numpy(Ks_np).to(device)
    # H5 stores c2w (Blender convention) and fov per view.
    from data_external.orbit import look_at_blender
    c2w_list = []
    for i in range(N_VIEWS):
        theta = 2 * np.pi * i / N_VIEWS
        elev = 0.4 * ORBIT_RADIUS if i % 2 == 0 else -0.1 * ORBIT_RADIUS
        eye = np.array(
            [ORBIT_RADIUS * np.cos(theta), elev, ORBIT_RADIUS * np.sin(theta)],
            dtype=np.float32,
        )
        c2w_list.append(look_at_blender(eye, np.zeros(3, dtype=np.float32),
                                          up=np.array([0, 1, 0], dtype=np.float32)))
    c2w_all = np.stack(c2w_list).astype(np.float32)
    fov_all = np.full(N_VIEWS, ORBIT_FOV_DEG, dtype=np.float32)

    # Load full-scene gsplat reference (already rendered by render_tomatoes_raw_orbit.py).
    full_imgs = []
    for i in range(N_VIEWS):
        p = FULL_GSPLAT_DIR / f"view_{i:02d}.png"
        full_imgs.append(imageio.v3.imread(p).astype(np.float32) / 255.0)
    full_imgs = np.stack(full_imgs)
    print(f"\nLoaded {N_VIEWS} full-scene gsplat references from {FULL_GSPLAT_DIR}/")
    ...
    # Step 5: per-N loop.
    for n_target in N_VARIANTS:
        print(f"\n========== N = {n_target} ==========")
        keep_idx = np.sort(rank[:n_target])

        pruned_ply = SCENE_DIR / f"pruned_n{n_target}.ply"
        h5_path = H5_DIR / f"tomatoes_n{n_target}.h5"
        gsplat_dir = RENDERS_DIR / f"gsplat_n{n_target}"
        gf_dir = RENDERS_DIR / f"gaussianformer_n{n_target}{tag}"
        ...
        pruned_imgs = render_gsplat_subset(arrays, keep_idx, viewmats, Ks, device)
        for i in range(N_VIEWS):
            imageio.v3.imwrite(gsplat_dir / f"view_{i:02d}.png",
                               (pruned_imgs[i] * 255).astype(np.uint8))
        print(f"  rendered {N_VIEWS} pruned-gsplat views -> {gsplat_dir}/")

        gf_imgs = render_gaussianformer(h5_path, pipeline, device)
        for i in range(N_VIEWS):
            imageio.v3.imwrite(gf_dir / f"view_{i:02d}.png",
                               (gf_imgs[i] * 255).astype(np.uint8))
        print(f"  rendered {N_VIEWS} GaussianFormer views -> {gf_dir}/")

        # PSNR (LDR, sRGB-clipped).
        psnr_vs_pruned = []
        psnr_vs_full = []
        for i in range(N_VIEWS):
        ...
        overview = make_3way_overview(full_imgs, pruned_imgs, gf_imgs)
        overview_path = RENDERS_DIR / f"overview_3way_n{n_target}{tag}.png"
    ...
    # Step 6: master overview comparing all N values for two representative views.
    print("\n========== MASTER OVERVIEW ==========")
    chosen_views = [0, 1]  # one high-elev view, one low-elev view
    rows = []
    for v in chosen_views:
        rows.append(make_master_overview(per_n, full_imgs, v))
    H = rows[0].shape[0]
    W = rows[0].shape[1]
    gutter = np.ones((8, W, 3), dtype=np.float32)
    master = np.concatenate([rows[0], gutter, rows[1]], axis=0)
    master_path = RENDERS_DIR / f"overview_all_N{tag}.png"
