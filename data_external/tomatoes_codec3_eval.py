...
h5 = Path("experiments/overfit/data/tomatoes_codec3/h5s/tomatoes_codec3.h5")
...
pipe = load_model(ModelSpec(ckpt=Path("checkpoints_tomato_codec3_r2/phase2_epoch_80.pt"), label="codec", pe_type="rope"), device)
...
iio.imwrite(D/"renders"/"codec3_verdict.png", (np.clip(np.concatenate(rows,0),0,1)*255).astype(np.uint8))
(D/"renders"/"codec3_verdict.json").write_text(json.dumps(summary, indent=1))
