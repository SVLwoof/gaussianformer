"""Codec v4 training data: 3x the view budget (4500 randomized views).

Codec v3 (1500 views, 2 cycles) reached near-parity with rec-GT (rand -1.62, close -0.06,
far -1.96) with the trajectory still unbent; views and cycles are the two levers that moved
it. v4 triples the view count at the same radius coverage (1.05-2.55) under a fresh seed.
Supervision stays free: targets rasterized from the FULL 219k splat, input = recovered 20k.
Eval sets are unchanged (codec_eval_*_c2w.npy, disjoint seed) so verdicts stay comparable.
"""
...
train_c2w = cams(4500, seed=41, radii=(1.05, 2.55))
troot = Path("experiments/overfit/data/tomatoes_codec4")
...
with h5py.File(troot/"h5s"/"tomatoes_codec4.h5", "w") as f:
...
render_set(train_c2w, troot/"renders", "tomatoes_codec4")
print(f"train: 4500 randomized views (r 1.05-2.55) -> {troot}")
