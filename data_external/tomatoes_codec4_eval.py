"""Codec v4 verdict: model vs rec-GT rasterization on held-out views incl. zoom extrapolation.

CKPT env overrides the checkpoint (default: cycle-2 final). Same eval sets as codec v1-v3.
"""
...
CKPT = Path(os.environ.get("CKPT", "checkpoints_tomato_codec4_r2/phase2_epoch_27.pt"))
TAG = os.environ.get("TAG", "codec4")
h5 = Path("experiments/overfit/data/tomatoes_codec4/h5s/tomatoes_codec4.h5")
...
iio.imwrite(D/"renders"/f"{TAG}_verdict.png", (np.clip(np.concatenate(rows,0),0,1)*255).astype(np.uint8))
(D/"renders"/f"{TAG}_verdict.json").write_text(json.dumps(summary, indent=1))
