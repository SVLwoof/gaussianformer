...
#   sbatch data_v10/train_nsweep_expand.sh
...
SEED=checkpoints_depthexpand/init_e18v9.pt
SAVE=checkpoints_nsweep_n100_expand
echo "EXPAND: 18L enc + 9L view (286.9M) | 4 GPUs, 300 epochs | node=$(hostname) sm_${ARCH}"
...
uv run --no-sync torchrun --standalone --nproc_per_node=4 -m training.train \
  ...
  --encoder_layers 18 --view_layers 9 \
