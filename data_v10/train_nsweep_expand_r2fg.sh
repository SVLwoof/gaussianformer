...
ENC=${ENC:?}
VIEW=${VIEW:?}
SEED=checkpoints_nsweep_n100_expand_r2/phase2_epoch_300.pt
SAVE=checkpoints_nsweep_n100_expand_r2fg
echo "EXPAND-R2 (fresh cosine restart on trained 18/9; disambiguates flat result: basin-stuck vs capacity-sufficient) e${ENC}v${VIEW} | 4 GPUs, 300 epochs | node=$(hostname) sm_${ARCH}"
...
uv run --no-sync torchrun --standalone --nproc_per_node=4 -m training.train \
  ...
  --encoder_layers $ENC --view_layers $VIEW \
  --phase2_epochs 300 --phase2_lr 5e-5 \
  --save_interval 20 --keep_last_n 2 \
  --fg_bg_weight 0.05 --log_loss_weight 0.5 --lpips_loss_weight 0.5 \
