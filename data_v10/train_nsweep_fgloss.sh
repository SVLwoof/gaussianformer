...
#SBATCH -c 16
#SBATCH --mem=64GB
#SBATCH --output=runs/nsweep_fgloss_%j.out
#SBATCH --job-name=nswfgloss
#SBATCH --gres=gg:g4:4
...
#   sbatch data_v10/train_nsweep_fgloss.sh
...
SAVE=checkpoints_nsweep_n100_fgloss
EPOCHS=300      # 100 scenes x 4 views / 4 ranks = 100 steps/epoch -> 30k steps, same as sweep
echo "FGLOSS: standard recipe + fg_bg_weight 0.05 | 4 GPUs, $EPOCHS epochs, node=$(hostname) sm_${ARCH}"
