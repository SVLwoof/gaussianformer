...
if [ "$ARM" = "objav" ]; then
  ...
  GH5=experiments/overfit/data/tomatoes_codec3/h5s; REN=experiments/overfit/data/tomatoes_codec3/renders
fi
SEED=checkpoints_v18_256/phase2_epoch_30.pt
SAVE=checkpoints_tomato_codec3${CYCLE2:+_r2}
if [ -n "$CYCLE2" ] && [ ! -d checkpoints_tomato_codec3_r2 ]; then
  SEED_OVERRIDE=checkpoints_tomato_codec3/phase2_epoch_80.pt
fi
