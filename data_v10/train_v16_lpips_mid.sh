...
#SBATCH -c 32
#SBATCH --mem=200GB
#SBATCH --output=runs/train_v16_lpips_mid_%j.out
#SBATCH --job-name=gformer_v16_lpips_mid
#SBATCH --gres=gg:g4:8
#SBATCH --account=sagieb
#SBATCH --killable
#SBATCH --requeue

# V16 MID-LPIPS fine-tune: intermediate of the lpips-weight sweep. Shares the hi run's log_w 0.5
# anchor, so vs train_v16_lpips_hi.sh the ONLY difference is lpips_w (0.5 vs 1.0) -> a clean
# comparison of perceptual pressure. The cranked (1.0) run looked grainy at ep2 on simple objects
# (over-strong?); this tests whether 0.5 keeps the detail gain with less graininess. Loss ratios
# across the sweep (lpips/log): dead-0.2 run = 0.2, mid = 1.0, hi = 2.0. --init_from v16_512 final.
#
#   sbatch data_v10/train_v16_lpips_mid.sh
...
SHM=/dev/shm/v16lpmid_${SLURM_JOB_ID}
TARS=data_v10/tars
trap "rm -rf $SHM" EXIT
...
files=( checkpoints_v16_lpips_mid/phase2_epoch_*.pt(Nom) )
...
uv run --frozen torchrun --standalone --nproc_per_node=8 -m training.train \
  ...
  --save_dir checkpoints_v16_lpips_mid \
