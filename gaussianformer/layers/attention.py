...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, src_key_padding_mask=None, rope_cos=None, rope_sin=None, rope_ctx_cos=None,
        ...
        if ATTN == 'sdpa' or force_sdpa:
            ...
            if self.is_self_attn:
                if src_key_padding_mask is not None:
                    q_unpad, indices_q, cu_seqlens_q, max_seqlen_q, _ = unpad_input(q.transpose(1, 2),
                                                                                    src_key_padding_mask)
                    k_unpad, indices_k, cu_seqlens_k, max_seqlen_k, _ = unpad_input(k.transpose(1, 2),
                                                                                    src_key_padding_mask)
                    v_unpad, indices_v, cu_seqlens_v, max_seqlen_v, _ = unpad_input(v.transpose(1, 2),
                                                                                    src_key_padding_mask)
...
class TransformerDecoder(nn.Module):
    def __init__(
        ...
        Args:
            ...
            pos_dim (int): dimension of the spatial position data.
